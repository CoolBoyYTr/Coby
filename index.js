 /*
* Coby es una creación de coolboy
* Coby no tiene ningun fin de lucro
* coolboy se reserva todos los derechos de autor
* © 2021 coolboy, INC.
...
/******FIN DE ARCHIVOS ANTILINK POR coolboy******/
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'Coby'; if (!author) author = 'coolboy';	
...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'Conectado by coolboy')
	})
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa Coby',
...
/******JUEGOS coolboy LA PUTA MADRE NO TE OLVIDES******/
...
/******JUEGOS coolboy LA PUTA MADRE NO TE OLVIDES******/					
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
				...
				break
		/*case 'tneon':
                data = await await getBuffer(`https://api.zeks.xyz/api/text3dbox?apikey=tcoolboyx&text=${body.slice(8)}`)
                if (!isUser) return reply(mess.only.daftarB)
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 MENCIONANDO 〙✪══\n╠➥'+teks+'╚═〘 by coolboy 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
			...
			//ANTILINKS DE REDES SOCIALES	
				...
				//FIN DE ANTILINK HECHO POR coolboy
	...
	//SERVICIO DE MUSICA Y VIDEO 			
		...
		if (!isUser) return reply(mess.only.daftarB)
	        reply(mess.only.musica2)
                play = body.slice(5)
                anu = await fetchJson(`https://api.zeks.me/api/ytplaymp3?q=${play}&apikey=coolboy46`)
                if (anu.error) return reply(anu.error)
                infomp3 = `*⌈ Canción Encontrada ✅ ⌉*\n◉ *Título:* ${anu.result.title}\n◉ *Fuente:* ${anu.result.source}\n◉ *Tamaño:* ${anu.result.size}\n\n*ESPERE ENVIANDO SU ARCHIVO MP3 ⚠*`
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_audio)
		...
		reply(mess.only.mpv)
		anu = await fetchJson(`https://api.zeks.me/api/ytmp4?url=${args[0]}&apikey=coolboy46`, {method: 'get'})
		if (anu.error) return reply(anu.error)
